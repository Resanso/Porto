          ...
          {skills.map((skill, idx) => (
            <div key={idx} className="flex flex-col items-center group/skill">
              <div className="relative mb-2">
                <skill.icon
                  className="w-10 h-10 text-slate group-hover/skill:text-neon-cyan transition-colors duration-300"
                  style={{ color: skill.color }}
                />
              </div>
              <span className="text-sm text-slate text-center group-hover/skill:text-light-slate transition-colors duration-300">
                {skill.name}
              </span>
            </div>
          ))}
