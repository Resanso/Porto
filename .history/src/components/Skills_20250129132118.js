...
const SkillCard = ({ skill, index }) => {
  ...
  return (
    ...
    >
      ...
      {/* Card Content */}
      <div className="card-glass relative p-4 sm:p-8 rounded-lg backdrop-blur-sm border border-white/10">
        {/* Skill Icon */}
        <div className="flex justify-center mb-4 sm:mb-6">
          ...
          >
            ...
            {/* Main icon */}
            <skill.icon
              className="w-8 h-8 sm:w-12 sm:h-12 relative z-10 transition-transform duration-300 group-hover:scale-110"
              style={{ color: skill.color }}
            />
          </motion.div>
        </div>
        ...
        <div className="text-center relative z-10">
          <h3 className="font-mono text-base sm:text-xl mb-2 sm:mb-3 text-white group-hover:text-neon-cyan transition-colors">
            {skill.name}
          </h3>
          <div className="h-1 sm:h-1.5 bg-navy-lighter rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
          ...
          <motion.span
            ...
            className="text-xs sm:text-sm text-slate mt-1 sm:mt-2 inline-block"
          >
            {skill.level}%
          </motion.span>
        </div>
...
const Skills = () => {
  ...
  return (
    <section
      id="skills"
      className="py-16 sm:py-32 bg-navy-light relative overflow-hidden"
    >
      ...
      <div className="max-w-6xl mx-auto px-4 relative">
        ...
        {/* Desktop View */}
        <div className="hidden md:grid grid-cols-3 gap-12 max-w-4xl mx-auto">
        ...
        <div className="relative md:hidden">
          <div className="skills-carousel overflow-x-auto hide-scrollbar flex gap-4 snap-x snap-mandatory pb-6">
            {skills.map((skill, index) => (
          ...
          <button
            ...
            className="absolute left-0 top-1/2 -translate-y-1/2 bg-navy-lighter/80 p-2 rounded-full text-neon-cyan hover:bg-navy-lighter"
          >
            ←
          </button>
          <button
            onClick={() => handleScroll("right")}
            className="absolute right-0 top-1/2 -translate-y-1/2 bg-navy-lighter/80 p-2 rounded-full text-neon-cyan hover:bg-navy-lighter"
          >
            →
          </button>
        </div>
